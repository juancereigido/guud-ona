...
void fadeOut (uint8_t fadeSpeed) {
  ...
  if (bright > 0) {
    ...
    if (bright <= 255 && bright > 80) { // If the brightness is between 255 and 80, the brightness will decrease twice as fast (the LEDs doenst visually change too much between these values)
      EVERY_N_MILLISECONDS(fadeSpeed) {
        bright = bright - 2;
        for(int i=0; i<=63; i++) {            // Turns ALL leds on, adding one step of brightness every N millisecons
          leds[i] = CRGB(bright,bright,bright);
...
void printValues () {
  ...
  brillo = "Brightness: " + String(bright);
